...
export function CandidateSelectorModal({
  isOpen,
  role,
  availableCandidates,
  currentTop3Ids,
  ...
  const filtered = availableCandidates.filter((c) => {
    if (c.id && currentTop3Ids.includes(c.id)) return false; // Exclude already in top 3
          ...
          >
            <div className="bg-white rounded-2xl shadow-xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col">
              <div className="flex items-center justify-between p-6 border-b border-gray-200">
                <div>
                  <h3 className="text-2xl font-bold text-gray-900">Select Candidate for</h3>
                  <p className="text-lg text-primary font-semibold">{role.title}</p>
                </div>
                <button
                  onClick={onClose}
